<div>
    <div class="mb-6 flex justify-between items-center">
        ...
        @if(!$showForm) <button wire:click="create" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-semibold">+ Add Customer</button> @endif
    </div>
    ...
    @if($showForm)
        <div class="bg-white rounded-lg shadow-md p-6 mb-6">
            <h2 class="text-xl font-bold mb-4">{{ $editingId ? 'Edit Customer' : 'New Customer' }}</h2>
            <form wire:submit.prevent="save">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="md:col-span-2"><label class="block text-sm font-semibold mb-2">Address</label><textarea wire:model="address" rows="2" class="w-full px-4 py-2 border rounded-lg"></textarea></div>
                </div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-x-auto">
        <table class="w-full">
            <thead class="bg-gray-50"><tr><th class="text-left py-3 px-4 font-semibold">Name</th><th class="text-left py-3 px-4 font-semibold">Phone</th><th class="text-right py-3 px-4 font-semibold">Actions</th></tr></thead>
            <tbody>
                @forelse($customers as $customer)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="py-3 px-4 text-right">
                            <button wire:click="edit({{ $customer->id }})" class="text-blue-600 hover:text-blue-800 mr-3">Edit</button>
                            <button wire:click="delete({{ $customer->id }})" onclick="return confirm('Are you sure?')" class="text-red-600 hover:text-red-800">Delete</button>
                        </td>
                    </tr>
                @empty <tr><td colspan="3" class="py-8 text-center text-gray-500">No customers found.</td></tr> @endforelse
            </tbody>
        </table>
